...
app.post('/api/register-meal', (req, res) => {
    ...
    // Xóa bản ghi cũ trong DB (nếu có) trước khi chèn mới
    db.run(`DELETE FROM meal_registrations WHERE username = ? AND date = ?`, [username, date], (err) => {
        db.run(
            `INSERT INTO meal_registrations (username, date, trua, toi, isLichTruc, createdAt) VALUES (?, ?, ?, ?, ?, ?)`,
            [username, date, trua ? 1 : 0, toi ? 1 : 0, isLichTruc ? 1 : 0, new Date().toISOString()],
            (err) => {
...
app.get('/api/meals', (req, res) => {
    db.all(`SELECT * FROM meal_registrations`, [], (err, rows) => {
        const meals = rows.map(m => ({
            ...m,
            trua: Boolean(m.trua),
            toi: Boolean(m.toi),
            isLichTruc: Boolean(m.isLichTruc)
        }));
        res.json(meals);
    });
});
